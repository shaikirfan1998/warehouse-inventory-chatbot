...
def enrich_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Available IS the real quantity — use it as the main stock number
    df["stock_qty"] = df["available"]
...
def df_to_documents(df: pd.DataFrame) -> list:
    ...
    for _, row in df.iterrows():
        content = (
            ...
            f"Quantity available: {row['available']}. "
    ...
    docs.append(Document(
        page_content=(
            ...
            f"total quantity available across all items: {df['available'].sum()}, "
